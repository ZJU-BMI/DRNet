...
def pdist2(X,Y):
    ...
    D = (C + ny.T) + nx
    # 两两之间的点积乘以-2，在加上两个向量各自的平方和，最后对结果开根号
...
def cf_nn(x, t):
    ...
    nn_t = Ic[np.argmin(D,0)] #为每个t=1找一个最接近的t=0
    nn_c = It[np.argmin(D,1)] #为每个t=0找一个最接近的t=1
...
def pehe_nn(yf_p, ycf_p, y, x, t, nn_t=None, nn_c=None):
    ...
    ycf_t = 1.0*y[nn_t]
    eff_nn_t = ycf_t - 1.0*y[It] # t=0 - t=1,其中t=0是挑出来的跟t=1特征上最接近的
    eff_pred_t = ycf_p[It] - yf_p[It] #预测的 0-1
...
def evaluate_result(result, data, validation=False,
    ...
    if 'loss' in result.keys() and result['loss'].shape[1]>=6:
        ...
        if validation:
            objective = np.array([losses[np.int((n_loss_outputs*i)/n_outputs),6,:] for i in range(n_outputs)]).T
        else:
            objective = np.array([losses[np.int((n_loss_outputs*i)/n_outputs),0,:] for i in range(n_outputs)]).T
...
def evaluate(output_dir, data_path_train, data_path_test=None, binary=False):
    ...
    for k in keys:
        eval_dict['train'][k] = np.array([eval_results[i]['train'][k] for i in range(len(eval_results))])

        eval_dict['valid'][k] = np.array([eval_results[i]['valid'][k] for i in range(len(eval_results))])

        if eval_test is not None and k in eval_results[0]['test']:
            eval_dict['test'][k] = np.array([eval_results[i]['test'][k] for i in range(len(eval_results))])
