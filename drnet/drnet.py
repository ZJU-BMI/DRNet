...
from drnet.util import *


class drnet(object):
    """
    drnet implements the DRNet proposed by J. Chu et. al. "On learning disentangled representations for individual \
    treatment effect estimation" (under review)

    The code of DRNet is built upon the Counterfactual regression (CFR) work of Johansson, Shalit & Sontag (2016) and \
    ...
    def _build_graph(self, x, t, y_, p_t, FLAGS, r_alpha, r_lambda, do_in, do_out, dims):
        ...
        self.t = t
        self.y_ = y_
        self.p_t = p_t  # proportion of treated
        self.r_alpha = r_alpha  # coefficient of IPM term in CFR. For DRNet,it should be 0.
        self.r_lambda = r_lambda  # coefficient of L2 regularization
        self.do_in = do_in  # keep_prob of input layers
        self.do_out = do_out  # keep_prob of output layers

        dim_input = dims[0]  # dimension of input feature
        dim_in = dims[1]  # size of input layers
        dim_out = dims[2]  # size of output layers
        ...
        #  build the outcome-oriented representation network
        for i in range(0, FLAGS.n_in):
            ...
            if FLAGS.varsel and i == 0:
                ...
                h_in.append(self.nonlin(z))  # apply non-linear activation function
                h_in[i + 1] = tf.nn.dropout(h_in[i + 1], do_in)  # apply dropout

        h_rep = h_in[len(h_in) - 1]  # take the output of the last layer

        #  build the outcome-oriented representation network with the same structure
        for i in range(0, FLAGS.n_in):
            if i == 0:
                ''' If using variable selection, first layer is just rescaling'''
                if FLAGS.varsel:  # false
                    weights_T_in.append(tf.Variable(1.0 / dim_input * tf.ones([dim_input])))
                else:
                    weights_T_in.append(tf.Variable(
                        tf.random_normal([dim_input, dim_in], stddev=FLAGS.weight_init / np.sqrt(dim_input))))
            else:
                weights_T_in.append(
                    tf.Variable(tf.random_normal([dim_in, dim_in], stddev=FLAGS.weight_init / np.sqrt(dim_in))))
            ...
            if FLAGS.varsel and i == 0:
                biases_T_in.append([])
                h_T_in.append(tf.mul(h_T_in[i], weights_T_in[i]))
            else:  # bias 初始化为0
                biases_T_in.append(tf.Variable(tf.zeros([1, dim_in])))
                z = tf.matmul(h_T_in[i], weights_T_in[i]) + biases_T_in[i]

                if FLAGS.batch_norm:  # 不做归一
                    batch_mean, batch_var = tf.nn.moments(z, [0])

                    if FLAGS.normalization == 'bn_fixed':
                        z = tf.nn.batch_normalization(z, batch_mean, batch_var, 0, 1, 1e-3)
                    else:
                        bn_biases_T.append(tf.Variable(tf.zeros([dim_in])))
                        bn_scales_T.append(tf.Variable(tf.ones([dim_in])))
                        z = tf.nn.batch_normalization(z, batch_mean, batch_var, bn_biases[-1], bn_scales[-1], 1e-3)

                h_T_in.append(self.nonlin(z))
                h_T_in[i + 1] = tf.nn.dropout(h_T_in[i + 1], do_in)
        h_T_rep = h_T_in[len(h_T_in) - 1]

        if FLAGS.normalization == 'divide':  # 输入层最后一层的输出做归一化，除以其标准差
            h_rep_norm = h_rep / safe_sqrt(tf.reduce_sum(tf.square(h_rep), axis=1, keep_dims=True))
            h_T_rep_norm = h_T_rep / safe_sqrt(tf.reduce_sum(tf.square(h_T_rep), axis=1, keep_dims=True))
        else:
            h_rep_norm = 1.0 * h_rep
            h_T_rep_norm = 1.0 * h_T_rep

        ''' Construct ouput layers '''
        # build two separate predictor for outcome predcition of treat and control
        y, weights_out, weights_pred = self._build_output_graph(h_rep_norm, t, dim_in, dim_out, do_out, FLAGS)
        # build the treatment classifier
        pred_t, weights_T_out, weights_T_pred = self._build_output(h_T_rep_norm, dim_in, dim_out, do_out, FLAGS)
        ...
        if FLAGS.loss == 'l1':
            ...
            res = y_ * tf.log(y) + (1.0 - y_) * tf.log(1.0 - y)  # modified cross entropy, for binary outcome like Jobs

            risk = -tf.reduce_mean(sample_weight * res)
            pred_error = -tf.reduce_mean(res)
        else:  # L2
            risk = tf.reduce_mean(sample_weight * tf.square(y_ - y))
            pred_error = tf.sqrt(tf.reduce_mean(tf.square(y_ - y)))  # RMSE, for numeric outcome like IHDP
        ...
        if FLAGS.imb_fun == 'mmd2_rbf':
            ...
            imb_error = r_alpha * mmd2_lin(h_rep_norm, t, p_ipm)  # use MMD in CFR. As for our prposed DRNet, r_alpha=0
            ...
            imb_error = r_alpha * imb_dist  # use WASS in CFR. As for our prposed DRNet, r_alpha=0
        ...
        if FLAGS.p_alpha > 0:
            tot_error = tot_error + imb_error  # As for DRNET, imb_error = 0

        if FLAGS.p_lambda > 0:
            tot_error = tot_error + r_lambda * self.wd_loss  # add the L2 regularization on trainable parameters

        if FLAGS.varsel:
            self.w_proj = tf.placeholder("float", shape=[dim_input], name='w_proj')
            self.projection = weights_in[0].assign(self.w_proj)

        self.t_classif_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=pred_t,
                                                                                     labels=self.t))
        # orthogonal loss
        orth_loss = tf.reduce_mean(tf.abs(
            tf.divide(tf.reduce_sum(tf.multiply(h_rep_norm, h_T_rep_norm), 1),
                      tf.multiply(tf.sqrt(tf.reduce_sum(tf.square(h_rep_norm), 1)),
                                  tf.sqrt(tf.reduce_sum(tf.square(h_T_rep_norm), 1))
                                  )
                      )
        ))

        tot_error = tot_error + FLAGS.coef_t_classif_loss * self.t_classif_loss
        tot_error = tot_error + FLAGS.coef_orth_loss * orth_loss
    ...
    def _build_output(self, h_input, dim_in, dim_out, do_out, FLAGS):
        ...
        for i in range(0, FLAGS.n_out):  # build hidden layers with L2  regularization on parameters
            ...
            h_out.append(self.nonlin(z))  # 施加激活函数
            h_out[i + 1] = tf.nn.dropout(h_out[i + 1], do_out)  # 施加dropout
        ...
        if FLAGS.varsel or FLAGS.n_out == 0:
            ...
            self.wd_loss += tf.nn.l2_loss(weights_pred)  # 预测层权重施加L2正则
    ...
    def _build_output_graph(self, rep, t, dim_in, dim_out, do_out, FLAGS):
        ...
        if FLAGS.split_output:
            ...
            # build two separate predictors for treated and control, respectively
            ...
            y = tf.dynamic_stitch([i0, i1], [y0, y1])  # order with the original index
