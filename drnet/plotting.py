...
def table_str_bin(result_set, row_labels, labels_long=None, binary=False):
    ...
    for i in range(len(result_set)):
        ...
        stds = [np.std(result_set[i][c])/np.sqrt(result_set[i][c].shape[0]) for c in cols] # 这里STD错了，多除以一个根号样本数
...
def select_parameters(results, configs, stop_set, stop_criterion, choice_set, choice_criterion):
    ...
    sort_key = np.argsort([np.mean(r[choice_set][choice_criterion]) for r in results_all]) # 根据pehe_nn对参数组合的结果排序
